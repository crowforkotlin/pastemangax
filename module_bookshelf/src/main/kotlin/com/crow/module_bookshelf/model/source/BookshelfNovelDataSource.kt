 ...
 * @formatter:on
 **************************/
class BookshelfNovelDataSource(inline val mDoOnPageResults: suspend (position: Int, pageSize: Int) -> List<BookshelfNovelResults>?) : PagingSource<Int, BookshelfNovelResults>() {
    ...
    // 当刷新时调用
    override fun getRefreshKey(state: PagingState<Int, BookshelfNovelResults>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, BookshelfNovelResults> {
        val position = params.key ?: START_POSITION
        "position : $position\tpageSize ${params.loadSize}".logMsg()
        return try {
            val realDataList = mutableListOf<BookshelfNovelResults>().apply { addAll(mDoOnPageResults(position, params.loadSize) ?: return@apply) }
