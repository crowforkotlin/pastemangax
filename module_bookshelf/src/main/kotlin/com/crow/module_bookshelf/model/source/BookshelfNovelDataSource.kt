 ...
 * @formatter:on
 **************************/
class BookshelfNovelDataSource(inline val mDoOnPageResults: suspend (position: Int, pageSize: Int) -> BookshelfNovelResp?) : PagingSource<Int, BookshelfNovelResults>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, BookshelfNovelResults> {
        ...
        return try {
            ...
            // 返回数据
            mutableListOf<BookshelfNovelResults>().run {
                addAll(result.mList)
