...
class DiscoverComicHomeDataSource(inline val mDoOnPageResults: suspend (position: Int, pageSize: Int) -> DiscoverComicHomeResp?) : PagingSource<Int, DiscoverComicHomeResult>() {
    ...
    // 当刷新时调用
    override fun getRefreshKey(state: PagingState<Int, DiscoverComicHomeResult>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, DiscoverComicHomeResult> {
        ...
        return try {
            ...
            // 返回数据
            mutableListOf<DiscoverComicHomeResult>().run {
                addAll(result.mList)
