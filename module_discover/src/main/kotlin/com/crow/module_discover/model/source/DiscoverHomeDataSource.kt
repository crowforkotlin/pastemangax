...
class DiscoverHomeDataSource(inline val mDoOnPageResults: suspend (position: Int, pageSize: Int) -> DiscoverHomeResp?) : PagingSource<Int, DiscoverHomeResult>() {
    ...
    // 当刷新时调用
    override fun getRefreshKey(state: PagingState<Int, DiscoverHomeResult>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, DiscoverHomeResult> {
        ...
        return try {
            ...
            // 返回数据
            mutableListOf<DiscoverHomeResult>().run {
                addAll(result.mList)
