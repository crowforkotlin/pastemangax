...
class DiscoverNovelAdapter(
    ...
    override fun onBindViewHolder(vh: ViewHolder, position: Int) {
        ...
        vh.mAppGlideProgressFactory = AppGlideProgressFactory.createGlideProgressListener(item.mImageUrl) { _, _, percentage, _, _ -> vh.rvBinding.discoverProgressText.text = AppGlideProgressFactory.getProgressString(percentage) }
        Glide.with(vh.itemView.context)
            .load(item.mImageUrl)
            .listener(vh.mAppGlideProgressFactory?.getRequestListener())
            .transition(mGenericTransitionOptions.transition { _ ->
                vh.rvBinding.discoverRvImage.animateFadeIn()
                vh.rvBinding.discoverLoading.animateFadeOut().withEndAction { vh.rvBinding.discoverLoading.alpha = 1f }
                vh.rvBinding.discoverProgressText.animateFadeOut().withEndAction { vh.rvBinding.discoverProgressText.alpha = 1f }
            })
