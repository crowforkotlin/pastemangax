...
interface ComicService {
    ...
    @FormUrlEncoded
    fun addComicToBookshelf(
        @Field("comic_id") comicId: String,
        @Field("is_collect") isCollect: Int,
        @Field("_update") update: Boolean = true
    ): Flow<BaseNullableResultResp<Any?>>

    @POST(URL.NovelAddToBookshelf)
    @FormUrlEncoded
    fun addNovelToBookshelf(
        @Field("book_id") novelId: String,
        @Field("is_collect") isCollect: Int
    ): Flow<BaseNullableResultResp<Any?>>

    @GET(URL.ComicCommentList)
    fun getComicComment(
        @Query("chapter_id") chapterId: String,
        @Query("offset") position: Int,
        @Query("limit") pagesize: Int
    ): Flow<BaseResultResp<ComicCommentListResp>>

    @POST(URL.ComicCommentSubmit)
    @FormUrlEncoded
    fun submitComment(
