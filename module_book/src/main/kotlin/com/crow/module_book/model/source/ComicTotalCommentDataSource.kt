...
class ComicTotalCommentDataSource(inline val onResult: suspend (position: Int, pageSize: Int) -> ComicTotalCommentResp?) : PagingSource<Int, ComicTotalCommentResult>() {
    ...
    // 当刷新时调用
    override fun getRefreshKey(state: PagingState<Int, ComicTotalCommentResult>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ComicTotalCommentResult> {
        ...
        return try {
            ...
            // 返回结果
            mutableListOf<ComicTotalCommentResult>().run {
