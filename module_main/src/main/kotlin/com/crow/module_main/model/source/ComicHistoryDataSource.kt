...
class ComicHistoryDataSource(inline val mDoOnPageResults: suspend (position: Int, pageSize: Int) -> ComicHistoryResp?) :
    PagingSource<Int, ComicHistoryResult>() {
    ...
    // 当刷新时调用
    override fun getRefreshKey(state: PagingState<Int, ComicHistoryResult>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ComicHistoryResult> {
        ...
        return try {
            ...
            // 返回数据
            mutableListOf<ComicHistoryResult>().run {
                addAll(result.mList)
