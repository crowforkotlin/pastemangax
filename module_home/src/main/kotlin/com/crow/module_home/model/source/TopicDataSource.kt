package com.crow.module_home.model.source
 ...
 * @formatter:on
 **************************/
class TopicDataSource(inline val mPageResults: suspend (position: Int, pageSize: Int) -> TopicResp?) : PagingSource<Int, TopicResult>() {
    ...
    // 当刷新时调用
    override fun getRefreshKey(state: PagingState<Int, TopicResult>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, TopicResult> {
        ...
        return try {
            ...
            // 获取书架漫画结果集
            val result = mPageResults(position, params.loadSize) ?: return LoadResult.Page(data = mutableListOf(), null, null)

            // 下一个键 = 如果 当前位置 + 预加载页数 大于 总共的数量则返回空 否则 返回下一个起点
            val nextKey = if (nextPos > result.mTotal) null else nextPos

            // 返回数据
            mutableListOf<TopicResult>().run {
                addAll(result.mList)
