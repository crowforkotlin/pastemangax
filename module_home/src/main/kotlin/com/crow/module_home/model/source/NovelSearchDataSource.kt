 ...
 * @formatter:on
 **************************/
class NovelSearchDataSource(inline val mDoOnPageResults: suspend (position: Int, pageSize: Int) -> SearchNovelResp?) : PagingSource<Int, SearchNovelResult>() {
    ...
    // 当刷新时调用
    override fun getRefreshKey(state: PagingState<Int, SearchNovelResult>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, SearchNovelResult> {
        ...
        return try {
            ...
            // 返回数据
            mutableListOf<SearchNovelResult>().run {
                addAll(result.mList)
