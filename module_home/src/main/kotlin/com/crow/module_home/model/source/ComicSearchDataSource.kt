 ...
 * @formatter:on
 **************************/
class ComicSearchDataSource(inline val mDoOnPageResults: suspend (position: Int, pageSize: Int) -> SearchComicResp?) : PagingSource<Int, SearchComicResult>() {
    ...
    // 当刷新时调用
    override fun getRefreshKey(state: PagingState<Int, SearchComicResult>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, SearchComicResult> {
        ...
        return try {
            ...
            // 返回数据
            mutableListOf<SearchComicResult>().run {
                addAll(result.mList)
